...
提到C++的可变长高性能缓冲区，就不得不提vector了，算是STL中性能与易用性的平衡做的非常好的容器，其余的容器普遍都没有vector好用。

在服务器的消息传输中，消息长度不固定，可能多次传输，每次传输的长度也不固定。核心的目标是减少传输过程中的阻塞，通过批量处理数据，减少系统调用次数提高效率。包括输入和输出缓冲区，前者用于批量处理输入，后者用于储存尚未发送的数据。
