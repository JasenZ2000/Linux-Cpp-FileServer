...
Acceptor::Acceptor(EventLoop* _loop)
    ...
    servAddr = new InetAddress("127.0.0.1", 1234);
    printf("Server listening on fd: %d, IP: %s Port: %d\n",
        listenSock->getFd(), inet_ntoa(servAddr->getAddr().sin_addr), ntohs(servAddr->getAddr().sin_port));
...
void Acceptor::acceptConnection() {
    ...
    struct sockaddr_in addr = clnt_addr->getAddr();
    printf("new client fd %d! IP: %s Port: %d\n", clnt_sock->getFd(), inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
